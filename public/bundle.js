...
function _playSound() {
  _playSound = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(state) {
    var file_audio, audioToPlay;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 2:
            file_audio = '';
            _context.t0 = state;
            _context.next = _context.t0 === 'addPeer' ? 6 : _context.t0 === 'removePeer' ? 8 : _context.t0 === 'newMessage' ? 10 : _context.t0 === 'error' ? 12 : 14;
            break;
...
var peerLoockupUrl = 'https://extreme-ip-lookup.com/json/'; //const notifyBySound = true; // turn on - off sound notifications

var isWebRTCSupported = DetectRTC.isWebRTCSupported;
var isMobileDevice = DetectRTC.isMobileDevice;
var myChatAvatar = '/images/programmer.svg';
var friendChatAvatar = '/images/friend.svg';
var startTime;
var elapsedTime;
var signalingServerPort = 3080; // must be same of server PORT
...
var countTime = null; // left buttons
...
var leaveRoomBtn = null; // chat room elements
...
var msgerChat = null;
var msgerInput = null;
var msgerSendBtn = null; // chat room emoji picker

var msgerEmojiPicker = null;
var msgerEmojiHeader = null;
var msgerCloseEmojiBtn = null;
var emojiPicker = null; // my audio - video devices
...
var selectors = null; // my video element

var myVideo = null; // my conference Name
...
function getHtmlElementsById() {
  countTime = (0, _script.getId)('countTime');
  myVideo = (0, _script.getId)('myVideo'); // left buttons

  nbtn = (0, _script.getId)('mbtn');
  shareRoomBtn = (0, _script.getId)('shareRoomBtn');
  audioBtn = (0, _script.getId)('audioBtn');
  videoBtn = (0, _script.getId)('videoBtn');
  swapCameraBtn = (0, _script.getId)('swapCameraBtn');
  screenShareBtn = (0, _script.getId)('screenShareBtn');
  fullScreenBtn = (0, _script.getId)('fullScreenBtn');
  chatRoomBtn = (0, _script.getId)('chatRoomBtn');
  myDevicesBtn = (0, _script.getId)('myDevicesBtn');
  leaveRoomBtn = (0, _script.getId)('leaveRoomBtn'); // chat Room elements

  msgerDraggable = (0, _script.getId)('msgerDraggable');
  msgerHeader = (0, _script.getId)('msgerHeader');
  msgerClean = (0, _script.getId)('msgerClean');
  msgerEmojiBtn = (0, _script.getId)('msgerEmojiBtn');
  msgerClose = (0, _script.getId)('msgerClose');
  msgerChat = (0, _script.getId)('msgerChat');
  msgerInput = (0, _script.getId)('msgerInput');
  msgerSendBtn = (0, _script.getId)('msgerSendBtn'); // chat room emoji picker

  msgerEmojiPicker = (0, _script.getId)('msgerEmojiPicker');
  msgerEmojiHeader = (0, _script.getId)('msgerEmojiHeader');
  msgerCloseEmojiBtn = (0, _script.getId)('msgerCloseEmojiBtn');
  emojiPicker = (0, _script.getSl)('emoji-picker'); // my audio - video devices

  myDevices = (0, _script.getId)('myDevices');
  myDeviceHeader = (0, _script.getId)('myDeviceHeader');
  myDevicesCloseBtn = (0, _script.getId)('myDevicesCloseBtn');
  audioInputSelect = (0, _script.getId)('audioSource');
  audioOutputSelect = (0, _script.getId)('audioOutput');
  videoSelect = (0, _script.getId)('videoSource'); // my conference Name

  myVideoParagraph = (0, _script.getId)('myVideoParagraph');
}
/**
...
function getServerUrl() {
  return 'http' + (location.hostname == 'localhost' ? '' : 's') + '://' + location.hostname + (location.hostname == 'localhost' ? ':' + signalingServerPort : '');
}

function getRoomId() {
  var roomId = location.pathname.substring(1); // if not specified room id, create one random

  if (roomId == '') {
    roomId = makeId(12);
    var newurl = signalingServer + '/' + roomId;
...
function initPeer() {
  ...
  if (!isWebRTCSupported) {
    ...
    (0, _script.userLog)('error', 'This browser seems not supported WebRTC!');
    return;
  ...
  signalingSocket.on('connect', function () {
    ...
    if (localMediaStream) joinToChannel();else setupLocalMedia(function () {
      whoAreYou();
    });
  });
  /**
  ...
  function whoAreYou() {
    (0, _script.playSound)('newMessage');
    Swal.fire({
  ...
  function welcomeUser() {
    var myRoomUrl = window.location.href;
    (0, _script.playSound)('newMessage');
    Swal.fire({
      position: 'center',
  ...
  signalingSocket.on('disconnect', function () {
    ...
    for (var _peer_id in peerConnections) {
      peerConnections[_peer_id].close();
    }
  ...
  signalingSocket.on('addPeer', function (config) {
    ...
    console.log('iceServers', iceServers[0]); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection
    ...
    (0, _script.playSound)('addPeer'); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/onicecandidate
    ...
    peerConnections[peer_id].ontrack = function (event) {
      ...
      if (ontrackCount === 2) {
        ...
        remoteMediaStream = event.streams[0]; // print peers name

        var videoWrap = document.createElement('div');
        var remoteVideoParagraph = document.createElement('h3');
        var peerVideoText = document.createTextNode(peers[peer_id]);
        remoteVideoParagraph.appendChild(peerVideoText);
        videoWrap.appendChild(remoteVideoParagraph);
        var remoteMedia = document.createElement('video');
        videoWrap.className = 'video';
        videoWrap.appendChild(remoteMedia);
        remoteMedia.setAttribute('id', peer_id);
        remoteMedia.setAttribute('playsinline', true);
        remoteMedia.setAttribute('poster', '/images/p.jpg');
        ...
        document.body.appendChild(videoWrap); // attachMediaStream is a part of the adapter.js library
    ...
    if (config.should_create_offer) {
      console.log('Creating RTC offer to', peer_id); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createOffer

      peerConnections[peer_id].createOffer().then(function (local_description) {
        console.log('Local offer description is', local_description); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setLocalDescription

        peerConnections[peer_id].setLocalDescription(local_description).then(function () {
          ...
          (0, _script.userLog)('error', 'Offer setLocalDescription failed!');
        });
      }).catch(function (e) {
  ...
  signalingSocket.on('sessionDescription', function (config) {
    ...
    var peer_id = config.peer_id;
    var remote_description = config.session_description; // https://developer.mozilla.org/en-US/docs/Web/API/RTCSessionDescription

    var description = new RTCSessionDescription(remote_description); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setRemoteDescription

    peerConnections[peer_id].setRemoteDescription(description).then(function () {
      console.log('setRemoteDescription done!');

      if (remote_description.type == 'offer') {
        console.log('Creating answer'); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createAnswer

        peerConnections[peer_id].createAnswer().then(function (local_description) {
          console.log('Answer description is: ', local_description); // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setLocalDescription
          ...
          peerConnections[peer_id].setLocalDescription(local_description).then(function () {
            ...
            (0, _script.userLog)('error', 'Answer setLocalDescription failed!');
          });
        }).catch(function (e) {
  ...
  signalingSocket.on('iceCandidate', function (config) {
    var peer_id = config.peer_id;
    var ice_candidate = config.ice_candidate; // https://developer.mozilla.org/en-US/docs/Web/API/RTCIceCandidate
  ...
  signalingSocket.on('removePeer', function (config) {
    ...
    (0, _script.playSound)('removePeer');
  }); // show chat messages
...
function setupLocalMedia(callback, errorback) {
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
    ...
    localMediaStream = stream; // Setup localMedia

    var videoWrap = document.createElement('div'); // print my name on top video element

    var myVideoParagraph = document.createElement('h3');
    myVideoParagraph.setAttribute('id', 'myVideoParagraph');
    videoWrap.appendChild(myVideoParagraph);
    var localMedia = document.createElement('video');
    videoWrap.className = 'video';
    ...
    (0, _script.userLog)('error', 'This app will not work without camera/microphone access.');
...
function resizeVideos() {
  var numToString = ['', 'one', 'two', 'three', 'four', 'five', 'six'];
...
function handleVideoPlayerFs(videoId) {
  var videoPlayer = (0, _script.getId)(videoId); // handle Chrome Firefox Opera Microsoft Edge videoPlayer ESC
  ...
  videoPlayer.addEventListener('fullscreenchange', function (e) {
    ...
    if (!fullscreenElement) {
      ...
      isVideoOnFullScreen = false; // console.log("Esc FS isVideoOnFullScreen", isVideoOnFullScreen);
    }
  ...
  videoPlayer.addEventListener('click', function (e) {
    ...
    if (!isVideoOnFullScreen) {
      ...
      videoPlayer.style.pointerEvents = 'none'; // console.log("Go on FS isVideoOnFullScreen", isVideoOnFullScreen);
    } else {
      ...
      videoPlayer.style.pointerEvents = 'auto'; // console.log("Esc FS isVideoOnFullScreen", isVideoOnFullScreen);
    }
  });
}
/**
...
function getTimeToString(time) {
  ...
  return "".concat(formattedHH, ":").concat(formattedMM, ":").concat(formattedSS);
}
/**
...
function setShareRoomBtn() {
  shareRoomBtn.addEventListener('click', /*#__PURE__*/function () {
...
function setSwapCameraBtn() {
  navigator.mediaDevices.enumerateDevices().then(function (devices) {
    var videoInput = devices.filter(function (device) {
...
function setChatRoomBtn() {
  ...
  setChatRoomForMobile(); // open hide chat room

  chatRoomBtn.addEventListener('click', function (e) {
    if (noPeerConnections()) {
      (0, _script.userLog)('info', "Can't Open Chat Room, no peer connection detected");
      return;
    }
  ...
  msgerClean.addEventListener('click', function (e) {
    cleanMessages();
  }); // close chat room - show left button and time if hide

  msgerClose.addEventListener('click', function (e) {
    hideChatRoomAndEmojiPicker();
    checkCountTime();
  ...
  msgerSendBtn.addEventListener('click', function (e) {
    // prevent refresh page
    e.preventDefault();
    var msg = msgerInput.value; // empity msg
...
function setChatEmojiBtn() {
  if (isMobileDevice) {
    ...
    msgerEmojiBtn.addEventListener('click', function (e) {
      // prevent refresh page
      e.preventDefault();
      hideShowEmojiPicker();
    });
    msgerCloseEmojiBtn.addEventListener('click', function (e) {
      // prevent refresh page
      e.preventDefault();
      hideShowEmojiPicker();
    });
...
function setDevicesBtn() {
  myDevicesBtn.addEventListener('click', function (e) {
    hideShowAudioVideoDevices();
  });
  myDevicesCloseBtn.addEventListener('click', function (e) {
    hideShowAudioVideoDevices();
  });
...
function gotStream(stream) {
  ...
  window.stream = stream; // refresh my video to peers

  for (var peer_id in peerConnections) {
    // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/getSenders
    var sender = peerConnections[peer_id].getSenders().find(function (s) {
  ...
  stream.getVideoTracks()[0].enabled = true; // https://developer.mozilla.org/en-US/docs/Web/API/MediaStream

  var newStream = new MediaStream([stream.getVideoTracks()[0], localMediaStream.getAudioTracks()[0]]);
  localMediaStream = newStream; // attachMediaStream is a part of the adapter.js library

  attachMediaStream(myVideo, localMediaStream); // Refresh button list in case labels have become available
...
function gotDevices(deviceInfos) {
  ...
  for (var i = 0; i !== deviceInfos.length; ++i) {
    var deviceInfo = deviceInfos[i]; // console.log("device-info ------> ", deviceInfo);
...
function shareRoomUrl() {
  return _shareRoomUrl.apply(this, arguments);
}

function _shareRoomUrl() {
  _shareRoomUrl = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
    var tmpInput, ROOM_URL, isSupportedNavigatorShare, errorNavigatorShare;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            tmpInput.select(); // For mobile devices

            tmpInput.setSelectionRange(0, 99999);
            document.execCommand('copy');
            console.log('Copied to clipboard Join Link ', ROOM_URL);
            document.body.removeChild(tmpInput); // navigator share

            isSupportedNavigatorShare = false;
            errorNavigatorShare = false; // if supported
          ...
          case 16:
            (0, _script.userLog)('info', 'Room Shared successfully!');
            _context2.next = 22;
            break;
          ...
          case 19:
            ...
            errorNavigatorShare = true; // This feature is available only in secure contexts (HTTPS),
          ...
          case 22:
            ...
            if (!isSupportedNavigatorShare || isSupportedNavigatorShare && errorNavigatorShare) {
              Swal.fire({
                ...
                html: "\n      Send this link to all participants\n      <p style=\"color:rgb(8, 189, 89);\">" + ROOM_URL + "</p>\n      <br>",
                showClass: {
                  popup: 'animate__animated animate__fadeInDown'
                },
                hideClass: {
...
function swapCamera() {
  if (noPeerConnections()) {
    (0, _script.userLog)('info', "Can't Swap the Camera, no peer connection detected");
    return;
  }

  camera = camera == 'user' ? 'environment' : 'user';
  if (camera == 'user') useVideo = true;else useVideo = {
    facingMode: {
      exact: camera
    }
  }; // https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia
  ...
  navigator.mediaDevices.getUserMedia({
    ...
    camStream.getVideoTracks()[0].enabled = true; // https://developer.mozilla.org/en-US/docs/Web/API/MediaStream

    var newStream = new MediaStream([camStream.getVideoTracks()[0], localMediaStream.getAudioTracks()[0]]);
    localMediaStream = newStream; // attachMediaStream is a part of the adapter.js library
    ...
    (0, _script.userLog)('error', 'Error to swaping the camera');
  });
}
...
function toggleScreenSharing() {
  if (!isScreenStreaming) {
    if (noPeerConnections()) {
      (0, _script.userLog)('info', "Can't Toggle screen sharing, no peer connection detected");
      return;
    }
  }
  ...
  if (!isScreenStreaming) {
    ...
    screenMediaPromise = navigator.mediaDevices.getUserMedia(_constraints); // make sure to enable video

    videoBtn.className = 'fas fa-video';
  }

  screenMediaPromise.then(function (screenStream) {
    ...
    screenStream.getVideoTracks()[0].enabled = true; // https://developer.mozilla.org/en-US/docs/Web/API/MediaStream

    var newStream = new MediaStream([screenStream.getVideoTracks()[0], localMediaStream.getAudioTracks()[0]]);
    localMediaStream = newStream; // attachMediaStream is a part of the adapter.js library
    ...
    (0, _script.userLog)('error', 'Unable to share the screen');
  });
}
...
function setChatRoomForMobile() {
  if (isMobileDevice) {
    ...
    document.documentElement.style.setProperty('--msger-height', '40vh');
    document.documentElement.style.setProperty('--msger-width', '40vw');
  }
}
/**
...
function hideShowAudioVideoDevices() {
  if (!isAudioVideoDevicesVisible) {
    if (noPeerConnections()) {
      (0, _script.userLog)('info', "Can't setup devices, no peer connection detected");
      return;
    }
...
},{"core-js/stable":"../node_modules/core-js/stable/index.js","regenerator-runtime/runtime":"../node_modules/regenerator-runtime/runtime.js","./script":"script.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
